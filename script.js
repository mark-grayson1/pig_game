...
function generateDiceRoll() {
  ...
  diceImg.src = `https://mark-grayson1.github.io/pig-game/images/dice-${dice}.png`;
...
function checkWinner(params) {
  ...
  document.querySelector(`#wins--${activePlayer}`).textContent = `Wins: ${
    wins[activePlayer - 1]
  }`;
  document
  ...
  console.log(
    `Player ${activePlayer} wins the game! Total Wins: ${
      wins[activePlayer - 1]
    }`
  );
}
